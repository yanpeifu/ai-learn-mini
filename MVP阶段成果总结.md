...
> 最后更新：2026-09-17（阶段 M0 / M1 / M2 全部完成）
...
| 项 | 状态 |
| --- | --- |
| M3 业务接口：登录（真实 code2session + DEV 假登录）、大纲保存、关卡下发（**不含 answer**）、开始/作答/结算、记录列表、继续学习、举报、统计、限流 | 下一步立即做 |
...
| 杂项：`frontend/README.md` 待补；NutUI 尚未引入（M4 决定是否真的需要）；成本优化方案待 M6 决策 | 待办 |
