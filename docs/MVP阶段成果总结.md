...
> 最后更新：2026-09-18（M0–M5 完成，M6 自动化回归完成；真机测试与内测需要你在本机进行）
...
| M5 | 五个页面 1:1 还原（原型 18 屏状态） | ✅ 完成 | 五个页面关键状态均已用「真实后端 + 真实模型 + 手机视口」截图核对 |
| M6 | 联调与回归（异常路径 / 边界 / 真实冒烟 / 质量回归 / 成本） | ✅ 自动化部分完成 | 质量回归 100 分、成本 0.052–0.055 元/次、时延 41s |

## M6 联调与回归结果（2026-09-18）

| 回归项 | 结果 |
| --- | --- |
| 质检脚本自校验 `--reaudit` | ✅ 脚本与规则仍可用，平均分 98.2（3 个样例相对上次有变化，方向一致） |
| 真实样例 `quality_check.py --limit 2` | ✅ 生成成功 **2/2**，平均 **100.0 分**，0 错误 0 警告（27.0s / 20.5s） |
| 生产链路 `smoke_pipeline.py` 连续 2 次 | ✅ 每次都是 15 题 / 3 关各 5 题 / **0 致命 0 警告**；耗时 41.6s、41.3s；成本 **0.0519 / 0.0553 元** |
| 出题时延 | ✅ 41s，满足 PRD「出题生成 ≤60 秒」 |
| 成本 | ✅ **0.052–0.055 元/次完整学习**，已非常接近 PRD 的 ≤0.05 元目标（超出 4%–11%），**结论：暂不需要优化**（此前首测 0.0677 元属于首次调用的偏高样本） |
| 异常路径 | ✅ 已由单测覆盖：超时/坏 JSON/网络抖动分别重试、401/402/403 立即中止、重复提交幂等、出题失败不留脏数据、中途退出可恢复（`/api/attempt/ongoing`） |
| 边界 | ✅ 已覆盖：20 字 / 2000 字边界、只留 1 个知识点被拒、全部答错（0 星 + 薄弱点 + 安抚文案）、全部答对（三星） |
| 真机测试（iOS + Android） | ⬜ **需要你在本机进行**：安装微信开发者工具 → 打开 `frontend/` → 用测试号预览 → 在手机上各跑一遍 |
| 内测（5–10 人） | ⬜ 需要你组织（PRD M7-06） |
...
| 页面视觉验收（H5 + 真实后端 + 真实模型） | 6 态 / 5 页 | ✅ 首页空态（P1-1）、首页校验错误（P1-3）、大纲可编辑（P2-2）、答题页（P3-1 单选未提交：进度条 20% + 题型/难度标签 + 题干卡 + ABCD 选项）、结算页（P4-2 低分鼓励态：0 星 + 20% 大数字 + 安抚文案 + 待复习错题/薄弱知识点）、我的页（P5-1：用户卡 + 三格概览 + 记录列表 + 协议与版本） |
| 完整闭环走通 | 1 次 | ✅ 在浏览器里真实走通：输入 → 生成大纲（真实模型）→ 开始闯关（真实出题 15 题）→ 逐题作答 → 结算 → 我的记录 |
